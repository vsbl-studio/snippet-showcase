...
Starter theme based on Roots Sage 10. 

Few notable modifications to the original theme:

1. Function in `app/Provides/ThemeServiceProvider.php` to disable Sage death screen which is triggered by warnings. 
2. Several helper functions in `app/vsbl-setup.php` according to our team needs.
3. Empty css files for custom style structure.

Several functions use credentials from `.env` file.

Add VSBL development or client provided credentials for email seding:
```
SMTP_HOST=''
SMTP_PORT=''
SMTP_USERNAME=''
...
Add `REMOTE_UPLOAD_URL` to replace local url with remote url for uploads.

# Default Roots Sage info
